...
function generateEmailHtml(entries: Record<Platform, any[]>, runAt: string): string {
  ...
  return `
    ...
    <head>
      <meta charset="utf-8">
      <title>Weekly New Creators - ${date}</title>
    </head>
    <body style="margin: 0; padding: 20px; background-color: #f7f7f7; font-family: Inter, system-ui, -apple-system, sans-serif;">
      <div style="max-width: 800px; margin: 0 auto; background-color: #ffffff; padding: 32px; border-radius: 8px;">
        <h1 style="color: #1a1a1a; margin-bottom: 8px;">🚀 Weekly New Top 200 Creators</h1>
        ...
        <p style="color: #999; font-size: 12px; margin: 0;">
          This data was automatically generated by the weekly-ingest function.
...
serve(async (req) => {
  ...
  try {
    ...
    if (DRY_RUN) {
      ...
      console.log('Subject:', `${totalNew} New Top 200 Creators - ${date}`);
      ...
      return new Response(
        JSON.stringify({
          ...
          dry_run: true,
          to: toEmail,
          subject: `${totalNew} New Top 200 Creators - ${date}`,
          html
        }),
        {
